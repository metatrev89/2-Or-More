...
- Next (Stage 2 remaining): R2 + backend deploy → model bake-off + vendor keys → RevenueCat trial webhook → app live-mode (swap mock adapters, real playback, push→Player) + Profile settings sub-pages + live forgot-password. Stage 3 = TestFlight/production (incl. Apple/Google SSO).
