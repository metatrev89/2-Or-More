...
- Catch-all + NO boilerplate affirmations (DECIDED Sept 9, 2026): after the seven areas the interviewer asks ONCE whether anything else belongs in the practice (`LifeArea 'open_capture'`, backend areaIndex 7, DB migration 0004). A real answer becomes the optional 8th goal/affirmation; a decline ("no", "that's it" — `DECLINE` regex in `services/intake.ts`) ends the intake at seven. `open_capture` is NOT a chakra: it stays out of `LIFE_AREAS`/`AREAS`, never lights a progress segment, and earns no star or chime — which is what keeps the 7-segment bar and the areaIndex contract intact. `AffirmationService.identityStatements()` was DELETED: two hardcoded lines ("I am positive…", "I stay in God's presence…") used to be appended to every set and surfaced as a generic #8 and #9 the user never said. Every affirmation must trace back to something the user actually told us — never staple set-level boilerplate onto the end again.
